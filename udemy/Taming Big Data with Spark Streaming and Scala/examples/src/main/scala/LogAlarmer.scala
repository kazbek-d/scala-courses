...
object LogAlarmer extends App {
  ...
  val statuses = lines
    .map(pattern.matcher)
    .map(matcher => if (matcher.matches) matcher.group(6) else "[error]")

  private val success = "Success"
  private val failure = "Failure"
  private val results = List(success, failure)
