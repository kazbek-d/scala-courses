...
object StackOverflow extends StackOverflow {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val vectors = vectorPostings(scored).persist(StorageLevel.MEMORY_AND_DISK_SER)  // [question.LangIndex * langSpread, answer.max_score]
    // assert(vectors.count() == 2121822, "Incorrect number of vectors: " + vectors.count())

    // {Calc LangIndex} = question.LangIndex * langSpread
    // mean =  { {Calc LangIndex}, {Reservoir sampling}.max_score }
    // sampleVectors(vectors)   - initial mean
...
class StackOverflow extends Serializable {
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val closest = vectors.map(p => (findClosest(p, means), p)) // ( Index of means, vector )
    val closestGrouped = closest.groupByKey()  // key = Index of means, Value = Iterable[vector]
