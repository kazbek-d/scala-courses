...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    while (index < len) {
      maxTg = max(input(index) / index.toFloat, maxTg)
      output.update(index, maxTg)
      index += 1
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    var index = 0
    var maxTg = Float.MinValue
    val len = math.min(input.length, until)
    while (from + index < len) {
      maxTg = max(input(from + index) / (from + index).toFloat, maxTg)
      index += 1
    }
    maxTg
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int, threshold: Int): Tree =
    if(end == from) Leaf(from, end, upsweepSequential(input, from, end))
    else if(end - from <= threshold || threshold == 0) {
      Leaf(from, end, upsweepSequential(input, from, end))
    } else {
      val mid = end + (from - end)/2
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int): Unit = {
    ...
    val len = input.length
    while (from + index < len && from + index < until) {
      maxTg = max(input(from + index) / (from + index).toFloat, maxTg)
      output.update(from + index, maxTg)
      index += 1
    }
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float, tree: Tree): Unit = tree match {
    ...
    case node@Node(left, righ) => parallel(downsweep(input, output, startingAngle, left), downsweep(input, output, left.maxPrevious, righ))
  }
