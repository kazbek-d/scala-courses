...
object StructuredStreaming extends App {
  ...
  val spark = SparkSession
    ...
    .config("spark.sql.streaming.checkpointLocation", tempFolder + "checkpoint")
    .getOrCreate()

  setupLogging()

  // Create a stream of text files dumped into the logs directory
  //val rawData = spark.read.text("./src/main/resources/logs/")
  ...
  val windowed = structuredData.groupBy($"status", window($"dateTime", "10 hour")).count().orderBy("window")

  //windowed.rdd.collect().foreach(println)
